...
- When looping back from a user's `askQuestions` selection, the Phase 1 content MUST directly and accurately address the specific option or freeform answer the user gave. Do not drift to a different topic.

## Phase 2 — Ask Next Step

After Phase 1 is fully complete, call the `askQuestions` tool with a single question:
...
- **NEVER** end a turn without calling `askQuestions`.
- **NEVER** render next-step options as plain text — always use the tool.
- **NEVER** call `askQuestions` before Phase 1 content is fully output.
