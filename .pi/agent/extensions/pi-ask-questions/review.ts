...
export function renderReview(options: ReviewRenderOptions) {
	...
	const safeSelectedPrompt = sanitizeDisplayText(selectedQuestion.prompt);
	...
	if (width >= 72) {
		...
		const leftLines = questions.map((question, i) => {
			...
			const raw = `${selected ? ">" : " "} ${box} ${sanitizeDisplayText(question.label)}`;
		...
		for (const line of wrapTextWithAnsi(theme.fg("text", safeSelectedPrompt), rightWidth)) {
			rightLines.push(line);
		}
		rightLines.push("");
		rightLines.push(theme.fg("muted", "Answer:"));
		for (const line of wrapTextWithAnsi(theme.fg(hasCurrentAnswer(selectedQuestion, answers, inputBuffers) ? "success" : "dim", sanitizeDisplayText(previewCurrentAnswer(selectedQuestion, answers, inputBuffers))), rightWidth)) {
			rightLines.push(line);
		}
		...
		for (let i = 0; i < questions.length; i++) {
			...
			const selected = i === reviewIdx;
			const box = hasCurrentAnswer(question, answers, inputBuffers) ? "■" : "□";
			const text = ` ${selected ? ">" : " "} ${box} ${sanitizeDisplayText(question.label)}`;
			add(selected ? theme.fg("accent", text) : theme.fg("muted", text));
		}
		lines.push("");
		add(theme.fg("muted", " Question:"));
		add(theme.fg("text", ` ${safeSelectedPrompt}`));
		lines.push("");
		add(theme.fg("muted", " Answer:"));
		add(theme.fg(hasCurrentAnswer(selectedQuestion, answers, inputBuffers) ? "success" : "dim", ` ${sanitizeDisplayText(previewCurrentAnswer(selectedQuestion, answers, inputBuffers))}`));
	}
